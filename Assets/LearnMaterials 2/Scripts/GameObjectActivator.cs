...
{
    [SerializeField]
    [Tooltip("Список объектов, которым будет переключаться состояние")]
    private List<StateContainer> targets;
    [Space]
    [SerializeField]
    [Tooltip("Отрисовать связи с управляемыми объектами. Зелёная - будет включено. Красная - выключена.")]
    private bool debug;
